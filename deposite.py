...
class Deposit:
    def __init__(self):
        ...
        self.l1=Label(self.root, text="Deposit")
    ...
    def display(self):
        ...
        self.l6=Label(self.f1, text="Deposited Amount:")
        ...
        self.b2=Button(self.f1,text="deposit",command=self.deposit)
    ...
    def deposit(self):
        ...
        if(upbal>bal):
            ...
            m=messagebox.askyesno("VALIDATE"," Do you want to deposit ?")
            if (m):
                n=self.generateid()
                (s,t)=self.datetime()
                ...
                self.cursor.execute("insert into tbtransaction values(%s,%s,%s,'Deposits',%s,%s,%s)",(n,ano,ano,str(amt),s,t))
                self.conn.commit()
                messagebox.showinfo("VALIDATE","YOUR UPDATED BALANCE IS "+str(upbal))
        else:
            messagebox.showerror("ALERT","enter amount in positive")
    def generateid(self):
